...
const ClusterExplorer = () => {
    ...
    const [externalSnapshotData, setExternalSnapshotData] = useState(null);
	...
	// Add this function to handle snapshots loaded from ClusterSnapshotLoader
	const handleSnapshotLoaded = (snapshotData) => {
	  console.log('Snapshot loaded from external source:', snapshotData);

	  // Set the resources from the loaded snapshot
	  setResources(snapshotData.data);
	  ...
	  setSelectedCluster(customClusterDetails.id);
	  setClusterDetails(customClusterDetails);
	  setFetchedSnapshotName(snapshotData.snapshotFilename || 'external-snapshot');
	  setExternalSnapshotData(snapshotData);
	  setSnapshotLoaded(true);

	  // Optionally run best practices analysis
    ...
    const refreshSnapshot = async () => {
        ...
        if (externalSnapshotData) {
          setError("Cannot refresh an externally loaded snapshot");
          return;
        }
    };
	  ...
	  if (urlClusterId) {
	    ...
	    fetch(`http://localhost:8000/cluster/snapshot`, {
	      ...
	      headers: {
	        ...
	        ...(urlApiKey && { 'Authorization': `Bearer ${urlApiKey}` })
	      },
	      body: JSON.stringify({
	        cluster_id: urlClusterId,
	        region: urlRegion || 'US',
	    ...
	    .then(result => {
	      if (result.status === 'success' && result.data) {
	        // Create mockResult with actual data
	        const mockResult = {
	          data: result.data,
	          clusterId: urlClusterId,
              clusterName: urlClusterName || `Cluster ${urlClusterId.substring(0, 8)}...`,
	          region: urlRegion || 'US',
	          apiKey: urlApiKey,
	          snapshotFilename: result.snapshotFilename || (urlDate ? `snapshot-${urlDate}` : 'latest-snapshot')
	        };

	        // Process the loaded snapshot
	        handleSnapshotLoaded(mockResult);
	      } else {
	        throw new Error(result.message || 'Failed to load snapshot');
	      }
	    })
	    .catch(error => {
	      console.error('Error auto-loading snapshot:', error);
	      setError(`Failed to auto-load snapshot: ${error.message}`);
	    });
	  }
	}, []); // Empty dependency array ensures this runs only once on component mount

	// Second useEffect - runs when resources change to handle best practices analysis
	useEffect(() => {
	  if (resources && typeof resources === 'object' && Object.keys(resources).length > 0) {
...
if (!selectedCluster && !resources && !snapshotLoaded) {
  // If URL parameters are being processed, show loading screen instead of welcome screen
  if (urlParamLoading) {
    return (
      <div className="h-screen w-full flex items-center justify-center bg-gray-100">
        <div className="bg-white p-8 rounded-lg shadow-lg flex flex-col items-center">
          <Loader2 className="w-16 h-16 animate-spin text-blue-500 mb-4" />
          <p className="text-xl font-medium">Loading cluster snapshot...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen">
      <div className="w-full md:w-1/2 p-6 flex items-center justify-center">
        <WelcomeScreen onClusterSelect={handleClusterSelect} />
      </div>
    ...
    const renderResourceDetails = () => {
        if (selectedResource === 'resourcesearch') {
            return <ResourceSearch onResultClick={handleResourceSearchResult} />;
        }

        if (!selectedResource) return null;

        if (selectedResource === 'bestpractices') {
            return (
                <div className="p-4">
                    <BestPracticesDetailView
                        ...
                        onClose={() => setSelectedResource('overview')}
        ...
        if (selectedResource === 'insights') {
            return (
                <div className="p-4">
                    <InsightsView onResourceSelect={handleResourceSelect} />
                </div>
            );
        }

        if (resourceDetails.length === 0) return null;

        switch (selectedResource) {
            case 'nodes':
                return <NodeDetails items={resourceDetails} handleResourceSelect={handleResourceSelect}/>;
            case 'pods':
            ...
            case 'configmaps':
                return <ConfigMapDetails items={resourceDetails}/>;
            default:
                return <DefaultResourceDetails items={resourceDetails}/>;
        }
    };
